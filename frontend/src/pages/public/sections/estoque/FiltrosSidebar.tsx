...
export default function FiltrosSidebar({ filtros, onChange, marcas, categorias, cambios, combustiveis, cores, portasOpts, aberto, onFechar }: Props) {
  ...
  return (
    <>
      {aberto && (
        <div className="fixed inset-0 bg-black/40 z-[99] lg:hidden" onClick={onFechar} />
      )}

      <aside
        className={[
          'bg-white self-start sticky top-4 rounded-xl shadow-sm border border-[#ebebeb]',
          'min-w-[300px] max-w-[300px] max-h-[calc(100vh-2rem)] overflow-y-auto',
          'max-[1200px]:fixed max-[1200px]:top-0 max-[1200px]:right-0',
          'max-[1200px]:w-[85vw] max-[1200px]:min-w-0 max-[1200px]:max-w-none',
          'max-[1200px]:max-h-screen max-[1200px]:z-[1000]',
          'max-[1200px]:transition-transform max-[1200px]:duration-300',
          'max-[1200px]:rounded-none max-[1200px]:shadow-2xl',
          aberto ? 'max-[1200px]:translate-x-0' : 'max-[1200px]:translate-x-full',
        ].join(' ')}
      >
        {/* Header fixo */}
        <div className="sticky top-0 bg-white z-10 flex items-center justify-between px-4 py-3 border-b border-[#f0f0f0]">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              type="button"
              className="hidden max-[1200px]:flex w-7 h-7 items-center justify-center rounded-full bg-[#f4f4f4] text-[#555] text-sm font-bold hover:bg-[#eee] transition-colors"
              onClick={onFechar}
            >
              ✕
            </button>
